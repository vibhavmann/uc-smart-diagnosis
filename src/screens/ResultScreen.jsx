    ...
    return (
      <div className="flex flex-col h-full bg-white dark:bg-gray-900">
        <div className="px-4 pt-10 pb-4 border-b border-gray-100 dark:border-gray-800 flex-shrink-0">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Helps us send the right pro</p>
        </div>
        <div className="flex-1 scroll-hide px-4 pt-6">
          <div className="bg-gray-50 dark:bg-gray-800 rounded-2xl p-5 mb-5 slide-up">
            <span className="text-2xl block mb-3">🤔</span>
            <p className="text-sm font-semibold text-gray-900 dark:text-white mb-1">{result.clarifyingQuestion}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400">Your answer ensures the right price too</p>
          </div>
          <div className="flex flex-col gap-3">
            {(result.clarifyingOptions || []).map((opt, i) => (
              <button
                key={opt}
                className="w-full text-left py-3.5 px-4 rounded-2xl border text-sm font-medium text-gray-800 dark:text-gray-200 bg-white dark:bg-gray-800 slide-up"
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: 'var(--c-bg)' }}>
      ...
      <div className="flex-1 scroll-hide px-4 pt-3 pb-3 space-y-3">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700 slide-up">
          <div className="flex items-start gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {result.variant && <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{result.variant}</p>}
            </div>
          </div>
          <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed mb-3">{result.why}</p>
        ...
        {/* price band */}
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700 slide-up" style={{ animationDelay: '60ms' }}>
          <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-2">Realistic Price Estimate</p>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-3 leading-relaxed">{result.priceBreakdown}</p>
          <div className="bg-amber-50 dark:bg-amber-950/40 rounded-xl px-3 py-2.5 flex items-start gap-2">
        ...
        {/* before / after */}
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden slide-up" style={{ animationDelay: '100ms' }}>
          <button className="w-full flex items-center justify-between px-4 py-3.5" onClick={() => setOpenBA(!openBA)}>
            ...
            <span className="text-gray-400 dark:text-gray-500 text-sm">{openBA ? '▲' : '▼'}</span>
          </button>
          {openBA && (
            <div className="px-3 pb-4 fade-in">
              <div className="grid grid-cols-2 gap-2.5">
                ...
                <div className="bg-emerald-50 dark:bg-emerald-950/40 rounded-xl p-3">
                  <p className="text-[11px] font-bold text-emerald-700 dark:text-emerald-400 mb-2">✅ New Path</p>
                  {['User describes problem', 'AI routes correctly', 'Right pro dispatched', 'Honest price upfront', 'No surprises', 'First-visit success', '5-star review'].map((t) => (
                    <p key={t} className="text-[11px] text-emerald-700 dark:text-emerald-400 leading-snug mt-1">{t}</p>
                  ))}
                </div>
              </div>
            </div>
        ...
        {/* impact */}
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden slide-up" style={{ animationDelay: '140ms' }}>
          <button className="w-full flex items-center justify-between px-4 py-3.5" onClick={() => setOpenImpact(!openImpact)}>
            ...
            <span className="text-gray-400 dark:text-gray-500 text-sm">{openImpact ? '▲' : '▼'}</span>
          </button>
          {openImpact && (
